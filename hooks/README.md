# obsidian-wiki super setup — Hooks

Plugin lifecycle hooks for the wiki vault. All hooks defined in `hooks.json`.
...
**Workaround:** the command-type SessionStart hook (`cat wiki/hot.md`) is the canonical safety check. As a fallback, copy the hook config from `hooks.json` into your user-level `~/.claude/settings.json` instead of relying on plugin hooks.
