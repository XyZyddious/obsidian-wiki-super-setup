---
name: canvas
description: Visual layer of the wiki. Add images, text cards, PDFs, and wiki pages to Obsidian canvas files (`.canvas` JSON) with auto-positioning inside zones. Integrates with /banana for image capture. Canvas nodes don't carry the universal frontmatter (canvas files are JSON), but any embedded wiki pages do. Triggers on /canvas, canvas new, canvas add image/text/pdf/note, canvas zone, canvas list, canvas from banana, add to canvas, put this on the canvas, open canvas, create canvas.
---
